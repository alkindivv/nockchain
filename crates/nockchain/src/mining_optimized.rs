...
impl OptimizedMiningState {
    async fn new() -> Self {
        ...
        let pool_size = (num_cores * 2).min(16); // Limit to reasonable number
    ...
    async fn return_kernel(&self, kernel: Kernel, temp_dir: TempDir) {
        let mut pool = self.kernel_pool.lock().await;
        if pool.len() < 16 { // Don't let pool grow too large
            pool.push_back((kernel, temp_dir));
        }
...
pub fn create_optimized_mining_driver(
    ...
    Box::new(move |mut handle| {
        Box::pin(async move {
            ...
            info!("Optimized mining state initialized");

            let mut next_candidates: VecDeque<NounSlab> = VecDeque::new();
            let (candidate_tx, mut candidate_rx) = mpsc::unbounded_channel::<NounSlab>();
            let (result_tx, mut result_rx) = mpsc::unbounded_channel::<NounSlab>();

            // Spawn parallel mining workers
            let num_workers = num_cpus::get().min(8); // Reasonable limit
            info!("Starting {} parallel mining workers", num_workers);

            for worker_id in 0..num_workers {
                let worker_candidate_rx = candidate_rx.clone();
            ...
            // Drop the original receivers to avoid keeping them alive
            drop(candidate_rx);
            drop(result_tx);
                ...
                tokio::select! {
                    effect_res = handle.next_effect() => {
                        ...
                        if effect_cell.head().eq_bytes("mine") {
                            ...
                            // Send to worker pool
                            if let Err(_) = candidate_tx.send(candidate_slab) {
                                warn!("Failed to send candidate to worker pool");
                            }
                        }
                    },
